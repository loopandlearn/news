...
The graphic below shows the paper cover for an InPlay BLE pod.

![the paper cover of an InPlay pod with Atlas highlighted](img/inplay-pod-atlas.png){width="400"}

* The fine-print item highlighted in the graphic above ends in `-Atlas` for `InPlay BLE` and in `-SAW` for `TWI BOARD`. In the US, you see a prefix of `FCC ID: RBV`. In other words, `FCC ID: RBV-Atlas` or  `FCC ID: RBV-SAW`. Other countries may have a different prefix.
