...
[AutoSENS](https://docs.diy-trio.org/settings/configuration/concepts/autosens-dynamic#autosens){:target="_blank"} is a dynamic adjustment mechanism for insulin sensitivity that operates on a moment-to-moment basis within [*Trio*](https://docs.diy-trio.org/index){:target="_blank"}. It's important to understand its key features:
...
* AutoSENS should not be confused with [AutoTUNE](https://docs.diy-trio.org/settings/configuration/autotune#autotune){:target="_blank"}, which serves a different purpose
